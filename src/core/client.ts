import type { ClickUpClient as IClickUpClient } from './types.js';
import {
  AuthError,
  NotFoundError,
  ValidationError,
...
export class ClickUpClient implements IClickUpClient {
  ...
  async request<T>(options: {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': `clickup-cli/${VERSION}`,
    };
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        try {
          ...
          errorMessage = parsed.err || parsed.error || parsed.ECODE || errorBody;
        } catch {
        ...
        switch (response.status) {
          ...
          case 429: {
            ...
            const waitSeconds = isNaN(retryAfter) ? undefined : Math.max(retryAfter - now, 1);
            const err = new RateLimitError(errorMessage, waitSeconds);
          ...
          default:
            ...
            throw new ClickUpError(errorMessage, 'API_ERROR', response.status);
        }
      } catch (error) {
        ...
        if (error instanceof TypeError && String(error.message).includes('fetch')) {
          throw new ClickUpError(`Network error: ${error.message}`, 'NETWORK_ERROR');
        }
    ...
    throw lastError ?? new ClickUpError('Request failed after retries', 'MAX_RETRIES');
  }
